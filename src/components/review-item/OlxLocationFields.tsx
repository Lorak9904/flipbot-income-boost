  ...
  return (
    <div className="grid gap-4 rounded-lg border border-neutral-700 bg-neutral-900/50 p-4 sm:grid-cols-2">
      <div className="space-y-2">
        ...
        {cities.length > 0 && (
          <div className="max-h-44 overflow-y-auto rounded-md border border-neutral-700 bg-neutral-950 p-1">
            {cities.map((city) => (
              <button key={city.id} type="button" className="block w-full rounded px-3 py-2 text-left text-sm text-neutral-200 hover:bg-neutral-800" onClick={() => chooseCity(city)}>
                {city.name}
              </button>
            ))}
          </div>
      ...
      <div className="space-y-2">
        <Label>{isPolish ? 'Dzielnica OLX' : 'OLX district'}</Label>
        <Select
        ...
        >
          <SelectTrigger><SelectValue placeholder={loadingDistricts ? (isPolish ? 'Wczytywanie…' : 'Loading…') : districts.length ? (isPolish ? 'Wybierz dzielnicę' : 'Select district') : (isPolish ? 'Brak dzielnic' : 'No districts')} /></SelectTrigger>
          <SelectContent>
            {districts.map((district) => <SelectItem key={district.id} value={String(district.id)}>{district.name}</SelectItem>)}
          </SelectContent>
        </Select>
        {districts.length > 0 && !value?.district_id && <p className="text-xs text-amber-300">{isPolish ? 'Dzielnica jest wymagana dla tego miasta.' : 'A district is required for this city.'}</p>}
      </div>
    </div>
  );
}
